...
const { storageAdapter } = require('@/lib/storage/storage-adapter');
...
const { DELETE: deletePost } = require('@/app/api/posts/[id]/route');
...
test('Security: Unauthorized mutation to review is denied (403 Forbidden)', async () => {
  ...
  const patchRes = await updateReview(patchReq, { params: { id: review.id } });
  ...
  const deleteRes = await deleteReview(deleteReq, { params: { id: review.id } });
...
test('Security: Unauthorized mutation to community post/comment/images is denied (403 Forbidden)', async () => {
  ...
  const uploadRes = await uploadPostImage(uploadReq, { params: { id: post.id } });
  ...
  const delComRes = await deleteComment(delComReq, { params: { id: comment.id } });
  ...
  const delPostRes = await deletePost(delPostReq, { params: { id: post.id } });
