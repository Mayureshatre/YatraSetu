...
export async function GET(
  ...
  try {
    const destination = await db.getDestinationById(params.id);
    if (!destination) {
      const { response, status } = createErrorResponse(
        'NOT_FOUND',
        `Destination with id '${params.id}' not found`,
        undefined,
        404
      );
    ...
    const originLat = parseResult.success && parseResult.data.origin_lat !== undefined
      ? parseResult.data.origin_lat
      : 23.2599; // Default central anchor
    const originLng = parseResult.success && parseResult.data.origin_lng !== undefined
      ? parseResult.data.origin_lng
      : 77.4126;
    const originLabel = (parseResult.success && parseResult.data.origin_label) || 'Starting Point';
    const vehicleType = (parseResult.success && parseResult.data.vehicle_type) || 'car';
