  ...
  try {
    ...
    if (!destination) {
      const { response, status } = createErrorResponse(
        'NOT_FOUND',
        `Destination '${params.id}' not found`,
        undefined,
        404
      );
    ...
    const latitude = parseResult.success && parseResult.data.lat !== undefined
      ? parseResult.data.lat
      : destination.latitude;
    const longitude = parseResult.success && parseResult.data.lng !== undefined
