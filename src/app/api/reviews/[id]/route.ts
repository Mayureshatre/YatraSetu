...
export async function PATCH(
  ...
  try {
    ...
    const result = await db.updateReview(params.id, userId, parseResult.data);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await db.deleteReview(params.id, userId);
    ...
    return NextResponse.json({ data: { success: true, message: `Review '${params.id}' deleted successfully` } });
  } catch (error: any) {
    const { response, status } = createErrorResponse(
      'SERVER_ERROR',
